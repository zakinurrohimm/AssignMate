...
<head>
    ...
    <title>ASSIGNMATE - DISPATCH_NEW_ASSIGNMENT</title>
    ...
    <div class="relative z-20">
        ...
        <main class="max-w-3xl mx-auto px-6 pt-32 pb-20 w-full">
            ...
            <div class="flex items-center gap-2 text-xs mono-font text-zinc-500 mb-6 uppercase tracking-wider">
                ...
                <span>/</span>
                <span class="text-amber-400">[DISPATCH_TASK]</span>
            </div>
                ...
                <div class="border-b border-zinc-800 pb-5 mb-8 flex justify-between items-start sm:items-center gap-4">
                    <div>
                        <div class="flex items-center gap-2 mb-1">
                            ...
                            <span class="text-[10px] text-amber-500 mono-font uppercase tracking-widest">// TASK_DISPATCH_PROTOCOL</span>
                        </div>
                        <h2 class="text-lg sm:text-xl font-bold tracking-widest text-white uppercase mono-font">
                            BUAT TUGAS BARU
                        </h2>
                        <p class="text-xs text-zinc-400 mt-1">Input parameter penugasan dan tentukan batas waktu eksekusi.</p>
                    </div>
                    
                    <a href="/matkul/create" class="flex-shrink-0 border border-zinc-700 hover:border-amber-500 text-zinc-300 hover:text-amber-400 bg-zinc-950 px-3.5 py-2 text-xs font-bold uppercase tracking-wider transition mono-font rounded flex items-center gap-1.5 shadow-sm">
                        <span class="text-amber-400">+</span>
                        <span class="hidden sm:inline">REG. MATKUL</span>
                        <span class="sm:hidden">MATKUL</span>
                    </a>
                </div>
                ...
                <form action="/tugas" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        @if($mataKuliah->isEmpty())
                            <p class="text-[11px] text-amber-400/80 mono-font mt-2 flex items-center gap-1">
                                <span>ℹ</span> Belum ada mata kuliah terdaftar. 
                                <a href="/matkul/create" class="underline hover:text-amber-300">Daftarkan di sini</a>.
                            </p>
                        @endif
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_tugas" value="{{ old('nama_tugas') }}" placeholder="Contoh: Implementasi Dijkstra Algorithm & Laporan" 
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select name="prioritas" class="w-full bg-zinc-950 border border-zinc-800 text-zinc-100 text-sm rounded-lg p-3.5 focus:border-amber-500 focus:outline-none transition cursor-pointer appearance-none">
                                    <option value="Sedang" {{ old('prioritas', 'Sedang') == 'Sedang' ? 'selected' : '' }}>🟡 Sedang (Standard Priority)</option>
                                    <option value="Tinggi" {{ old('prioritas') == 'Tinggi' ? 'selected' : '' }}>🔴 Tinggi (High Priority)</option>
                                    <option value="Rendah" {{ old('prioritas') == 'Rendah' ? 'selected' : '' }}>🟢 Rendah (Low Priority)</option>
                                </select>
                                <span class="absolute right-4 top-4 text-zinc-500 pointer-events-none text-xs">▼</span>
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" rows="3" placeholder="Instruksi tugas, format submission, atau link repositori..." 
                                  class="w-full bg-zinc-950 border border-zinc-800 text-zinc-100 text-sm rounded-lg p-3.5 focus:border-amber-500 focus:outline-none transition placeholder-zinc-600 resize-none">{{ old('deskripsi') }}</textarea>
                    </div>
                    ...
                    <div class="flex flex-wrap items-center gap-4 pt-6 border-t border-zinc-800">
                        <button type="submit" class="bg-amber-500 hover:bg-amber-400 text-black px-6 py-3.5 text-xs font-bold uppercase tracking-wider transition rounded-lg shadow-[0_0_20px_rgba(245,158,11,0.25)] mono-font flex items-center gap-2 cursor-pointer">
                            <span>></span>
                            <span>[DISPATCH] SIMPAN TUGAS</span>
                        </button>
