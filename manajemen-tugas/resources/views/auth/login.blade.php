...
<head>
    ...
    <title>ASSIGNMATE - OPERATOR_LOGIN</title>
...
<body class="bg-[#0b0c10] text-slate-200 min-h-screen flex flex-col justify-center items-center relative selection:bg-amber-500 selection:text-black p-4 sm:p-6 overflow-x-hidden">
    ...
    <div class="w-full max-w-md bg-zinc-900/90 border border-zinc-800 rounded-xl p-8 relative z-20 shadow-[0_0_40px_rgba(0,0,0,0.7)] backdrop-blur-xl border-glow-amber">
        ...
        @if ($errors->any())
            <div class="mb-6 bg-rose-950/40 border border-rose-600/60 rounded-lg text-rose-300 p-4 text-xs mono-font shadow-[0_0_15px_rgba(225,29,72,0.15)]">
                <p class="font-bold text-rose-400 mb-1 flex items-center gap-1.5">
                    <span>[!]</span> [ACCESS_DENIED]: Autentikasi Gagal
                </p>
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-5">
            ...
            <div class="flex items-center justify-between pt-1">
                <label class="flex items-center gap-2 cursor-pointer select-none">
                    <input id="remember_me" type="checkbox" name="remember" class="w-4 h-4 bg-zinc-950 border-zinc-800 text-amber-500 rounded focus:ring-amber-500 focus:ring-offset-zinc-900 accent-amber-500">
                    <span class="text-xs text-zinc-400 mono-font uppercase">
            ...
            <div class="text-center pt-3 border-t border-zinc-800/80">
                <a href="{{ route('register') }}" class="text-xs text-zinc-400 hover:text-amber-400 mono-font transition uppercase tracking-wider inline-flex items-center gap-1.5">
                    <span>// INITIALIZE_NEW_OPERATOR</span>
